...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if self.setPoint > temp:
            while self.current_state != self.heat:
                self.send('cycle')
        if self.setPoint < temp:
            while self.current_state != self.cool:
                self.send('cycle')
        if self.setPoint == temp:
    ...
    def setupSerialOutput(self):
        output = f"State: {self.current_state}, Temp: {self.getFahrenheit}, Setpoint: {self.setPoint}"        
        return output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                lcd_line_2 = f"Temp: {str(self.getFahrenheit())}"
    
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"{self.current_state}. Set:{self.setPoint}"
            ...
            if((counter % 30) == 0):
                ser.write(bytes(self.setupSerialOutput(),'utf-8'))
